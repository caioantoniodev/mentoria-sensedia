...
public class UpdateStave {
    ...
    public StaveDto update(String id, InputUpdateStaveDto inputUpdateStaveDto) {
        ...
        if (staveOptional.isEmpty()) throw new RuntimeException();

        var entity = staveOptional.get();

        if (!entity.getTheme().equals(inputUpdateStaveDto.getTheme())) {
            var stave = staveRepository.existsStaveByThemeAndState(inputUpdateStaveDto.getTheme(), "SESSION_VOTES_DONE");

            if (stave) throw new RuntimeException();
        }
